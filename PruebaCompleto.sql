...
permiso INT NOT NULL FOREIGN KEY REFERENCES Permiso(idpPermiso) ON
DELETE CASCADE
);
...
estado VARCHAR(20) NOT NULL CHECK(estado IN('estudio',
'administracion','post')),
...
idUsuario VARCHAR(15) NOT NULL FOREIGN KEY REFERENCES
Usuario(identificacion) ON DELETE CASCADE,
idfRol INT NOT NULL FOREIGN KEY REFERENCES Rol(idpRol) ON DELETE
CASCADE,
idfProyecto INT NOT NULL FOREIGN KEY REFERENCES
Proyecto(idpProyecto) ON DELETE CASCADE,
...
idfTarea INT NULL FOREIGN KEY REFERENCES Tarea(idpTarea) ON DELETE
NO ACTION,
nombre VARCHAR(100) NOT NULL,
duracionHora INT NOT NULL CHECK(duracionHora > 0),
descripcion TEXT NULL,
estado VARCHAR(15) NOT NULL CHECK(estado in('completado','en 
proceso','sin iniciar')),
...
idfUsuario VARCHAR(15) NOT NULL FOREIGN KEY REFERENCES
Usuario(identificacion) ON DELETE CASCADE,
idfArea INT NOT NULL FOREIGN KEY REFERENCES Area(idpArea) ON DELETE
CASCADE,
...
idfTarea INT NULL FOREIGN KEY REFERENCES Tarea(idpTarea) ON DELETE
CASCADE,
idfArchivo INT NULL FOREIGN KEY REFERENCES Archivo(idpArchivo) ON
DELETE CASCADE,
idfProyecto INT NOT NULL FOREIGN KEY REFERENCES
Proyecto(idpProyecto) ON DELETE CASCADE
);
...
idfRolUsuarioProyecto INT NOT NULL FOREIGN KEY REFERENCES
RolUsuarioProyecto(idpRolUsuarioProyecto) ON DELETE NO ACTION,
idfTareaArchivoProyecto INT NOT NULL FOREIGN KEY REFERENCES
TareaArchivoProyecto(idpTareaArchivoProyecto) ON DELETE NO ACTION,
...
1) Validar cuando se agrega un proyecto la fecha de inicio sea menor o 
igual al día actual, 
para evitar registren proyectos a futuro que puede y no sean 
concretados y permitir registrar
proyectos que se hayan iniciado anteriormente.
2) Validar que cuando se cumple un proyecto la fecha de cumplimiento sea 
mayor al día actual, 
para evitar que se registre un proyecto como cerrado en una fecha 
anterior a la fecha actual 
del sistema y se permita poner una fecha de cierre programada, de 
esta manera la fecha de inicio 
...
RAISERROR ( 'la fecha de inicio del proyecto debe ser menor o 
igual al dia actual',11,1);
ELSE IF SYSDATETIME () >= @fechaCierre 
RAISERROR ( 'la fecha de cierre del proyecto debe ser mayor al 
dia actual',11,1);
ELSE
...
1) Al registrar las horas de un usuario en la tabla RegistroHoraUsuario 
la fechaRegistro 
de dicha tabla debe estar entre la fechaInicio y la fechaCierre 
del respectivo proyecto
2) Validar que ese usuario y la tarea pertenezcan al mismo proyecto, para
evitar tener horas registradas
...
DECLARE @idfRolUsuarioProyecto INT, @idfTareaArchivoProyecto INT,
@fechaRegistro DATETIME,
@idpProyecto int, @idpProyectoFromUser int , @idUsuario 
VARCHAR(15), @fechaInicio DATETIME, @fechaCierre DATETIME, @msg varchar(10)
SELECT @idfRolUsuarioProyecto = idfRolUsuarioProyecto FROM INSERTED
SELECT @idfTareaArchivoProyecto = idfTareaArchivoProyecto FROM
INSERTED
SELECT @fechaRegistro = fechaRegistro FROM INSERTED
SELECT @idpProyecto = idfProyecto FROM TareaArchivoProyecto where
idpTareaArchivoProyecto = @idfTareaArchivoProyecto;
SELECT @idpProyectoFromUser = idfProyecto FROM RolUsuarioProyecto 
where idpRolUsuarioProyecto = @idfRolUsuarioProyecto;
...
VALUES(NEXT VALUE FOR secPermiso, @pCrear,@pLeer , @pBorrar,
@pActualizar);
END;
GO
...
BEGIN
INSERT INTO
Usuario(idpUsuario,identificacion,nombre,apellido,correo,telefono,costoHora)
VALUES(NEXT VALUE FOR secUsuario,@pIdentificacion ,@pNombre 
,@pApellido,@pCorreo,@pTelefono, @pcostoHora);
END;
GO
...
BEGIN
INSERT INTO
Proyecto(idpProyecto,codigo,nombre,siglas,estado,descripcion,fechaInicio,fechaCierre,costoCalculado,costoReal)
...
BEGIN
INSERT INTO
RolUsuarioProyecto(idpRolUsuarioProyecto,idUsuario,idfRol,idfProyecto,fechaAsignacion,fechaDesasignacion)
VALUES(NEXT VALUE FOR
secRolUsuarioProyecto,@pIdUsuario,@pIdfRol,@pidfProyecto,@pFechaAsignacion,@pFechaDesasignacion);
END;
GO
...
BEGIN
INSERT INTO
Tarea(idpTarea,idfTarea,nombre,duracionHora,descripcion,estado,prioridad,fechaAsignacion,fechaCumplimiento,costoCalculado,costoReal)
VALUES(NEXT VALUE FOR
secTarea,@pIdfTarea,@pNombre,@pDuracionHora,@pDescripcion,@pEstado,@pPrioridad,@pFechaAsignacion,@pFechaCumplimiento,@pCostoCalculado,@pCostoReal);
END;
GO
...
VALUES(NEXT VALUE FOR secArchivo,
@pCodigo,@pNombre,@pArchivo,@pObservacion);
END;
GO
...
BEGIN
INSERT INTO
TareaArchivoProyecto(idpTareaArchivoProyecto,idfTarea,idfArchivo,idfProyecto)
VALUES(NEXT VALUE FOR secTareaArchivoProyecto,@pIdfTarea 
,@pIdfArchivo,@pidfProyecto);
END;
GO
...
BEGIN
INSERT INTO
RegistroHoraUsuario(idpRegistroHoraUsuario,idfRolUsuarioProyecto,idfTareaArchivoProyecto,cantidadHoras,fechaRegistro,observacion)
...
SELECT * FROM Rol;EXEC uspInsertUsuario '100', 'Pedro', 'Gonzales', 'pedro@gmail.com',
11111111, 20000;
EXEC uspInsertUsuario '101', 'Felipe', 'Campos', 'felipe@gmail.com',
22222222, 20000;
EXEC uspInsertUsuario '102', 'Maria', 'Ramirez', 'maria@gmail.com',
33333333, 20000;
EXEC uspInsertUsuario '103', 'Sara', 'Salas', 'sara@gmail.com', 44444444,
17000;
EXEC uspInsertUsuario '104', 'Daniel', 'Villalobos', 'daniel@gmail.com',
55555555, 17000;
EXEC uspInsertUsuario '105', 'Monica', 'Hernandez', 'monica@gmail.com',
66666666, 10000;
EXEC uspInsertUsuario '106', 'Jose', 'Murillo', 'jose@gmail.com',
77777777, 10000;
EXEC uspInsertUsuario '107', 'David', 'Herrera', 'david@gmail.com',
88888888, 12000;
SELECT * FROM Usuario;

EXEC uspInsertProyecto 'P-AAAA-2020-1', 'Sistema Control de Costos',
'SCC', 'administracion', 'Sistema que lleve un control de los costos del 
area de Recursos Humanos, incluyecto los correspondientes a productos, 
servicios y empleados', '2020-04-11', NULL, 0, 0 ;
EXEC uspInsertProyecto 'E-BBBB-2022-1', 'Sistema Gestion de Proyectos',
'SGPTI', 'estudio', NULL, '2022-03-31', NULL, 0, 0;
EXEC uspInsertProyecto 'E-CCCC-2021-2', 'Sistema Gestion de Archivos',
'SGAR', 'estudio', NULL, '2021-05-30', NULL, 0, 0;
EXEC uspInsertProyecto 'E-CCCC-2021-3', 'Sistema Gestion de Cedulas',
'SGCE', 'estudio', NULL, '2021-08-30', NULL, 0, 0;
...
EXEC uspInsertRolUsuarioProyecto '107', 3, 2, '2021-05-22', '2022-04-17';
...
AS
BEGIN
UPDATE Usuario SET correo=@pCorreo, telefono = @pTelefono, costoHora =
@pcostoHora
...
AS
BEGIN
UPDATE Proyecto SET codigo = @pCodigo, nombre = @pNombre, estado =@pEstado, descripcion = @pDescripcion,
...
AS
BEGIN
UPDATE RolUsuarioProyecto SET idUsuario = @pIdUsuario, idfRol = @pIdfRol,
idfProyecto = @pidfProyecto,
...
AS
BEGIN
UPDATE Tarea SET idfTarea = @pIdfTarea,nombre = @pNombre, duracionHora =
@pDuracionHora,descripcion = @pDescripcion,estado = @pEstado,
 prioridad = @pPrioridad,fechaAsignacion =
@pFechaAsignacion,fechaCumplimiento = @pFechaCumplimiento,costoCalculado =
@pCostoCalculado,
...
AS
BEGIN
UPDATE Archivo SET codigo = @pCodigo,nombre = @pNombre,archivo = @pArchivo 
...
AS
BEGIN
UPDATE TareaArchivoProyecto SET idfTarea = @pIdfTarea,idfArchivo =
@pIdfArchivo,idfProyecto = @pidfProyecto
...
AS
BEGIN
UPDATE RegistrohoraUsuario SET idfRolUsuarioProyecto =
@pIdfRolUsuarioProyecto,idfTareaArchivoProyecto =
@pIdfTareaArchivoProyecto,
 cantidadHoras = @pCantidadHoras,fechaRegistro =
@pFechaRegistro,observacion=@pObservacion
...
SELECT * FROM Usuario;SELECT * FROM Proyecto;
EXEC uspUpdateProyecto 2,'E-DDD-2021-1', 'Sistema Gestion de Proyectos TI', 'estudio', NULL, '2022-05-30', 0, 0;
SELECT * FROM Proyecto;SELECT * FROM RolUsuarioProyecto;
